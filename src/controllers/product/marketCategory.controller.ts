...
export const linkMarketCategory = async (req: Request, res: Response) => {
  try {
    ...
    const existingMarketCategoryLink = await MarketCategory.findOne({ marketId, categoryId });

    if(existingMarketCategoryLink){
      return res.status(409).json({ message: "The link of marketId & categoryId is already exist" });
    }
